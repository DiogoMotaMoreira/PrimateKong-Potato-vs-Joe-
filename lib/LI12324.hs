...
-- | Peças possíveis para construir um 'Mapa'm.
data Bloco
...
{-| Função que gera uma Hitbox através de uma Personagem.
...
data Menu = EmJogo | MenuInicial | MenuVenceu | MenuPerder | Definicao | Pausa | Nivel
...
type ImagensMenus = [Picture]
type ImagensColec = [(Colecionavel,Picture)]
...
createHitbox:: Posicao -> (Double,Double) -> Hitbox
createHitbox (x,y) (c,l) = (((x-l/2),(y-c/2)) , ((x+l/2),(y+c/2)))

tamanhoJanela:: (Int,Int) -- (largura, altura) sem escala --- ao alterar este valor, os testes dão errado
tamanhoJanela = (20,20)  

pontoMatrizInicio :: (Float,Float)
pontoMatrizInicio = (-10,10)
